...
Vector<Data>::Vector(const ulong dim) {
    ...
    size = dim;
//    std::cout << "Ho creato Vector di dimensione: " << size << std::endl;
}
...
Vector<Data>::Vector(const Vector<Data>& vector) {
    ...
    /*
        std::copy(): in ordine i parametri sono:
        1) Primo dato da copiare.
        2) Posizione del dato subito dopo l'ultimo da copiare.
        3) I dati vanno copiati in Elements.
    */
...
Vector<Data>::~Vector() {
    delete[] Elements;
//    std::cout << "Oggetto Vector deallocato!" << std::endl;
}

// COPY ASSIGNMENT
template <typename Data>
Vector<Data>& Vector<Data>::operator=(const Vector<Data>& vector) noexcept {
    Vector<Data>* temp_vector = new Vector<Data>(vector); // Si sfrutta il costruttore di copia.
